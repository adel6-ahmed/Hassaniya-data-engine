...
const reviewPatchSchema = z.object({
  ...
  // Edit fields
  questionHassaniya: z.string().optional(),
  answerHassaniya: z.string().optional(),
...
export async function PATCH(req: NextRequest, { params }: Params) {
  try {
    ...
    let { action, reviewStatus, curationStage, questionHassaniya, answerHassaniya, answerFrench } = parsed.data
    if (action) {
    ...
    if (questionHassaniya !== undefined) updateData.questionHassaniya = questionHassaniya
    if (answerHassaniya !== undefined) updateData.answerHassaniya = answerHassaniya
    if (answerFrench !== undefined) updateData.answerFrench = answerFrench
...
export async function GET(_req: NextRequest, { params }: Params) {
  try {
    ...
    const item = await prisma.faqEntry.findUnique({
      where: { id: params.id },
    })
