...
const reviewPatchSchema = z.object({
  ...
  // Edit fields
  hassaniyaSentence: z.string().optional(),
...
export async function PATCH(req: NextRequest, { params }: Params) {
  try {
    ...
    let { action, reviewStatus, curationStage, hassaniyaSentence, msaTranslation, frenchTranslation } = parsed.data
    if (action) {
    ...
    if (hassaniyaSentence !== undefined) updateData.hassaniyaSentence = hassaniyaSentence
    if (msaTranslation !== undefined) updateData.msaTranslation = msaTranslation
...
export async function GET(_req: NextRequest, { params }: Params) {
  try {
    ...
    const item = await prisma.parallelSentence.findUnique({
      where: { id: params.id },
    })
